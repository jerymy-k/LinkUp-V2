<div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 h-[calc(100vh-12rem)]">
            ...
            <div
                class="lg:col-span-1 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-full">
                <!-- En-tête -->
                        ...
                        <div class="divide-y divide-gray-200">
                            @foreach ($conversations as $conversation)
                                ...
                                <a wire:click="selectConversation({{ $conversation->id }})"
                                    class="cursor-pointer block p-4 hover:bg-gray-50 transition-colors"
                                    data-conversation-id="{{ $conversation->id }}">
                                    <div class="flex items-center space-x-3">
                                        <!-- Avatar -->
                                        <div class="relative flex-shrink-0">
                                            @if ($conversation->friend->profile_photo)
                                                <img src="{{ Storage::url($conversation->friend->profile_photo) }}"
                                            ...
                                            @if ($conversation->friend->is_online)
                                                <div
            ...
            <div
                class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-full">
                @if (isset($selectedConversation))
                    ...
                    <div class="p-6 border-b border-gray-200">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-3">
                                <a href="{{ route('profile.show', $conversation->friend->id) }}"
                                    ...
                                    <div>
                                        <h2 class="font-semibold text-gray-900">{{ $conversation->friend->name }}</h2>
                                        <p class="text-sm text-gray-500">
                                            @if ($conversation->friend->is_online)
                                                <span class="text-green-600">● En ligne</span>
                                            @else
                                                Dernière connexion
                                                {{ $conversation->friend->last_seen?->diffForHumans() ?? 'inconnue' }}
                                            @endif
                                        </p>
                                    </div>
                                </a>
                            ...
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('profile.show', $conversation->friend->username) }}"
                        ...
                        <div class="py-1">
                            ...
                            <form method="POST" action="{{ route('conversations.block', $conversation->friend->id) }}">
                                @csrf
                                <button type="submit" 
                                onclick="return confirm('Bloquer {{ $conversation->friend->name }} ? Vous ne pourrez plus recevoir de messages de cette personne.')"
                            ...
                            <div class="flex {{ $isOwn ? 'justify-end' : 'justify-start' }}">
                                ...
                                <div class="flex flex-col {{ $isOwn ? 'items-end' : 'items-start' }} max-w-[70%]">
                                    <div class="group relative">
                                        <div
                                            class="px-4 py-2 rounded-2xl {{ $isOwn ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-900' }}">
                                            <p class="text-sm whitespace-pre-line break-words">{{ $message->content }}
                                            </p>
                                        </div>
                                        {{-- Image --}}
                                        @if ($message->image)
                                        ...
                                        @if ($message->video)
                                            <div class="mt-2">
                                                <video controls class="rounded-lg max-w-xs">
                                                    <source src="{{ asset('storage/' . $message->video) }}">
                                        ...
                                        @if ($isOwn)
                                            <div
                                                class="absolute top-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity -mt-1 -mr-1">
                    ...
                    <div class="p-6 border-t border-gray-200">
                        <form id="message-form" method="POST"
                            action="{{ route('messages.store', $selectedConversation->id) }}"
                            enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="receiver_id" value="{{ $conversation->friend->id }}">

                            <div class="flex items-end space-x-2">
                                <div class="flex-1 relative">
                                    <textarea name="content" id="message-input" rows="1" placeholder="Écrivez votre message..." required
                ...
                @else
                    ...
                    <div class="flex flex-col items-center justify-center h-full p-8">
                        ...
                        <p class="text-gray-500 text-center mb-8 max-w-md">
                            Sélectionnez une conversation dans la liste ou commencez une nouvelle discussion avec un
                            ami.
                        </p>
                    </div>
                @endif
        ...
        <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-lg bg-white">
            ...
            <div class="mb-4">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="search-friends" placeholder="Nom ou pseudo..."
                    ...
                    <div class="friend-item flex items-center justify-between p-3 rounded-lg hover:bg-gray-50"">
                        <div class="flex items-center space-x-3">
                            @if ($friend->profile_photo)
                                <img src="{{ Storage::url($friend->profile_photo) }}" alt="{{ $friend->name }}"
                                    class="h-10 w-10 rounded-full object-cover">
                            @else
